...
func (h *Handler) SetIsActive(c *gin.Context) {
	...
	if err != nil {
		h.l.Error(
			"users.setIsActive: service failed",
			slog.Any("err", err),
			slog.String("user", req.UserID),
		)
	...
	h.l.Info(
		"users.setIsActive: success",
		slog.String("user", u.UserID),
...
func (h *Handler) GetReview(c *gin.Context) {
	...
	if userID == "" {
		...
		c.JSON(
			...
			gin.H{"error": gin.H{"code": "BAD_REQUEST", "message": "user_id required"}},
		)
		return
	}
	...
	if err != nil {
		h.l.Error(
			"users.getReview: service failed",
			slog.Any("err", err),
