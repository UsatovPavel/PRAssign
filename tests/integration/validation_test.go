...
func TestSetIsActiveValidation(t *testing.T) {
	...
	status, body := postJSONWithToken(t, "/users/setIsActive", map[string]interface{}{"user_id": "no-such", "is_active": true}, adminToken)
...
func TestPRCreateValidation(t *testing.T) {
	...
	status, _ = postJSONWithToken(t, "/pullRequest/create", map[string]interface{}{"pull_request_id": "x"}, adminToken)
	if status == 201 {
