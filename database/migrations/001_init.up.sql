...
CREATE TABLE users (
    ...
    team_name VARCHAR(30) NOT NULL REFERENCES teams(team_name) ON DELETE CASCADE,
...
CREATE TABLE pull_requests (
    pull_request_id VARCHAR(30) PRIMARY KEY,
    pull_request_name VARCHAR(30) NOT NULL,
    author_id VARCHAR(30) NOT NULL REFERENCES users(user_id),
    status VARCHAR(30) NOT NULL CHECK (status IN ('OPEN','MERGED')) DEFAULT 'OPEN',
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    merged_at TIMESTAMPTZ NULL
);

CREATE TABLE pull_request_reviewers (
    pull_request_id VARCHAR(30) REFERENCES pull_requests(pull_request_id) ON DELETE CASCADE,
